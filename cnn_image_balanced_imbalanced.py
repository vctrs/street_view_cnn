...
thelist=open(r'C:\Users\victo\Desktop\data elective\images_segm_weimar\clusters_out.txt','r').readlines() 
...
thelist=thelist0[0:min([len(thelist0),len(thelist1)])]+thelist1[0:min([len(thelist0),len(thelist1)])]

#thelist=thelist0+thelist1

print(len(thelist),len([s for s in thelist if s.split('\n')[0].split(";")[1]=='0' ]),len([s for s in thelist if s.split('\n')[0].split(";")[1]=='1' ]),len([s for s in thelist if s.split('\n')[0].split(";")[1]=='2']),len([s for s in thelist if s.split('\n')[0].split(";")[1]=='-1' ]))


path1 = "C:\\Users\\victo\\Desktop\\data elective\\images_segm_weimar"
path2 = "C:\\Users\\victo\\Desktop\\data elective\\cnn_image_resize_weimar"

#listing = os.listdir(path1)
#num_smples=size(listing)
num_smples=len(thelist)
print("Total num of samples without noise: ",num_smples)

img_rows, img_cols = 200, 200

#for file in listing:
for file in [s.split('\n')[0].split(";")[0] for s in thelist+thelistm1]:
...
imlist_noise=[line.split('\n')[0].split(";")[0] for line in thelistm1 ]
...
immatrix = np.asarray([np.asarray(Image.open("C:\\Users\\victo\\Desktop\\data elective\\cnn_image_resize_weimar"+ '\\' + im2)).flatten()
                for im2 in imlist], 'f')

immatrix_noise=np.asarray([np.asarray(Image.open("C:\\Users\\victo\\Desktop\\data elective\\cnn_image_resize_weimar"+ '\\' + im2)).flatten()
                for im2 in imlist_noise], 'f')
...
#label_noise=np.random.randint(2, size=len(thelistm1))                               #assign random class to noise 
...
#input image dimensions        #####ALREADY DEFINED ABOVE
#img_rows, img_cols = 200, 200
...
noise_len+=noise_len%2

label_noise=shuffle(np.repeat([0,1], noise_len/2).flatten())     #asisgning 0 and 1 labels to noise. since the dataset is balanced the number of 0 and 1 should be equal
